...
export class ThemeManager {
    ...
    private static injectStyles() {
        ...
        // 添加时间戳防止缓存
        link.href =
            `scripts/extensions/Engram_project/dist/style.css?v=${Date.now()}`;
    ...
    private static applyThemeVariables(themeName: ThemeName) {
        ...
        const opacity = glassEnabled
            ? (settings.glassSettings?.opacity ?? 1)
            : 1;
        ...
        Object.entries(themeConfig.colors).forEach(([key, value]) => {
            ...
            if (shouldApplyTransparency && transparentKeys.has(key)) {
                ...
                finalValue =
                    `color-mix(in srgb, ${value}, transparent ${effectiveTransparency}%)`;
            }

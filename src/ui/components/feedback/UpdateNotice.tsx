 ...
 */
async function getExtensionInfo(): Promise<
    { name: string; type: "global" | "local" | "system" } | null
> {
    try {
        ...
        const extensions: { name: string; type: string }[] = await response
            .json();
...
async function updateEngramExtension(): Promise<
    { success: boolean; message: string; isUpToDate?: boolean }
> {
    try {
        ...
        const isGlobal = extInit?.type === "global" ||
            extInit?.type === "system";

...
export const SummaryReview: React.FC<SummaryReviewProps> = (
    ...
    const parseEvents = (): any[] => {
        ...
        if (content) {
            try {
                ...
                const cleanContent = content.replaceAll(/```(json)?/g, "")
                    .trim();
    ...
    useEffect(() => {
        ...
        const parentHasEvents = data && !Array.isArray(data) &&
            typeof data === "object" && Array.isArray((data as any).events);
    ...
    const handleAddEvent = () => {
        const isObjectFormat = events.length > 0 &&
            typeof events[0] === "object";
        const newItem = isObjectFormat
            ? { meta: {}, significance_score: 0.5, summary: "" }
            : "";
    ...
    const renderKV = (evt: any, idx: number) => {
        ...
        const fields = [
            {
                color: "text-value border-value/20 bg-value/5",
                key: "time_anchor",
                label: "时间",
            },
            {
            ...
            {
                color: "text-emphasis border-emphasis/20 bg-emphasis/5",
                key: "role",
                label: "人物",
            },
            {
        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="space-y-4 pr-2 pb-4">
                {events.map((evt, idx) => {
                    ...
                    const displayTitle = isObject
                        ? (evt.structured_kv?.event || evt.meta?.event ||
                            `Event ${idx + 1}`)
                        : `Event ${idx + 1}`;

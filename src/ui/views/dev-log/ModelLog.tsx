...
/** 状态图标 */
const StatusIcon: React.FC<{ status: ModelLogEntry["status"] }> = (
    { status },
) => {
    switch (status) {
        case "pending": {
        ...
        <div className="border border-border rounded-lg overflow-hidden bg-card">
            ...
            >
                ...
                {sent.character && (
                    ...
                    >
                        {sent.character.length > 8
                            ? sent.character.slice(0, 8) + "..."
                            : sent.character}
                    </span>
                )}
                <span className="text-xs text-muted-foreground">
                    {formatTime(sent.timestamp)}
                </span>

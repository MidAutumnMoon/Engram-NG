...
const DataBatchSection: React.FC = () => {
    ...
    const handleScan = async () => {
        ...
        try {
            ...
            const pending = allEvents.filter((e) =>
                e.is_embedded && !e.is_archived && e.level === 0
            ).length;
    ...
    const handleArchive = async () => {
        ...
        try {
            const store = useMemoryStore.getState();
            const allEvents = await store.getAllEvents();
            const toArchive = allEvents.filter((e) =>
                e.is_embedded && !e.is_archived && e.level === 0
            );
            ...
            <div className="p-4 rounded-lg bg-card/30 border border-border/30 space-y-3">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        disabled={isArchiving || !archiveStats ||
                            archiveStats.pending === 0}
                    >
                        {isArchiving
                            ? <Loader2 size={12} className="animate-spin" />
...
export const BatchProcessingPanel: React.FC = () => {
    ...
    const [endFloor, setEndFloor] = useState<number>(
        summarizerService.getStatus().currentFloor || 0,
    );
    ...
    const handleAnalyze = useCallback(async () => {
        ...
        try {
            ...
            const result = await batchProcessor.analyzeHistory(
                startFloor,
                endFloor,
                types,
            );
    ...
    const handleStart = useCallback(async () => {
        ...
        await batchProcessor.startHistory(
            analysis.startFloor,
            analysis.endFloor,
            types,
        );
    ...
    const overallPercent =
        queue?.overallProgress && queue.overallProgress.total > 0
            ? Math.round(
                (queue.overallProgress.current / queue.overallProgress.total) *
                    100,
            )
            : 0;
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
            ...
            <section className="space-y-6">
                <div>
                    <h2 className="text-sm font-medium text-heading mb-1">
                        历史消息批处理
                    </h2>
                    ...
                    <div className="space-y-4 pt-2">
                        ...
                        <div>
                            ...
                            <div className="flex justify-between mt-1 text-[11px] text-meta font-mono">
                                <span>
                                    当前阶段: {queue.currentTaskIndex + 1} /
                                    {" "}
                                    {queue.overallProgress.total}
                                </span>
                                <span
                                    className={queue.isPaused
                                        ? "text-emphasis"
                                        : (queue.isRunning
                                            ? "text-primary"
                                            : "")}

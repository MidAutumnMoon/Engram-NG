...
export interface UseWorldInfoReturn {
    ...
    updateProfile: (
        id: string,
        updates: Partial<WorldbookConfigProfile>,
    ) => void;
...
export function useWorldInfo(): UseWorldInfoReturn {
    const [worldbookStructure, setWorldbookStructure] = useState<
        Record<string, any[]>
    >({});
    ...
    const [currentCharWorldbook, setCurrentCharWorldbook] = useState<
        string | null
    >(null);
    ...
    const toggleWorldbook = useCallback((name: string, disabled: boolean) => {
        setDisabledWorldbooks((prev) => {
            ...
            setWorldbookConfig((prevConfig: WorldbookConfig | undefined) =>
                prevConfig ? { ...prevConfig, disabledWorldbooks: next } : {
                    ...getDefaultAPISettings().worldbookConfig,
    ...
    const toggleEntry = useCallback(
        (worldbook: string, uid: number, disabled: boolean) => {
    ...
    const updateProfile = useCallback(
        (id: string, updates: Partial<WorldbookConfigProfile>) => {

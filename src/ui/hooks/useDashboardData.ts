...
export function useDashboardData(refreshInterval = 2000): DashboardData & {
    ...
    const fetchMemoryStats = useCallback(async () => {
        ...
        if (
            currentMod !== lastDbModified.current ||
            lastDbModified.current === 0
        ) {
            ...
            const archivedCount = await db.events.filter((e) =>
    ...
    const refresh = useCallback(async () => {
        try {
            ...
            Logger.error(LogModule.DASHBOARD, "刷新 Dashboard 数据失败", {
                error,
            });
            if (isMounted.current) setIsLoading(false);
        }
    ...
    const toggleFeature = useCallback(async (feature: keyof FeatureStatus) => {
        ...
        switch (feature) {
            ...
            case "embedding": {
                const embeddingConfig = currentApiSettings.embeddingConfig ||
                    defaults.embeddingConfig!;
            ...
            case "recall": {
                const recallConfig = currentApiSettings.recallConfig ||
    ...
    useEffect(() => {
        ...
        const scheduleTimer = () => {
            ...
            const currentInterval = isTabActive
                ? refreshInterval
                : refreshInterval * 5; // 后台延长 5 倍间隙

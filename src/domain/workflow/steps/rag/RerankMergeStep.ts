...
export class RerankMergeStep implements IStep {
    ...
    async execute(context: JobContext): Promise<void> {
        ...
        const keywordCandidates: ScoredEvent[] =
        ...
        const hardLimit = activeConfig.keywordTopK?.events ??
            activeConfig.embedding?.topK ?? 50;
        ...
        if (activeConfig.useRerank && rerankService.isEnabled()) {
            ...
            try {
                const rerankResults = await rerankService.rerank(
                    rerankQuery,
                    documents,
                );

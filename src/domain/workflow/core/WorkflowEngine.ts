    ...
    static async run(
        ...
        try {
            ...
            for (let i = 0; i < workflow.steps.length; i++) {
                ...
                if (context.signal && context.signal.cancelled) {
                    Logger.warn(LogModule.RAG_INJECT, "工作流被中途取消", {
                        jobId: context.id,
                    });
                    break;
                }
                ...
                Logger.debug(LogModule.RAG_INJECT, `执行步骤: ${step.name}`, {
                    jobId: context.id,
                });
                ...
                try {
                    ...
                    if (result) {
                        ...
                        if (result.action === "abort") {
                            throw new Error(
                                result.reason || "Step requested abort",
                            );
                        }

                        if (result.action === "jump") {
                            const targetIndex = stepIndexMap.get(
                                result.targetStep,
                            );
                            if (targetIndex === undefined) {
                                throw new Error(
                                    `Jump target not found: ${result.targetStep}`,
                                );
                            }

                            // P0 Fix: 无限循环防护 (保险丝)
                            context.metadata.jumpCount =
                                (context.metadata.jumpCount || 0) + 1;
                            ...
                            Logger.debug(
                                LogModule.RAG_INJECT,
                                `跳转步骤: ${step.name} -> ${result.targetStep}`,
                                {
                    ...
                    Logger.error(
                        ...
                        `步骤执行崩溃: ${step.name}`,
                        {
                            error: stepError instanceof Error
                                ? stepError.message
                                : String(stepError),
                            stack: stepError instanceof Error
                                ? stepError.stack
                                : undefined,
                        },
                    );
            ...
            context.metadata.error = error instanceof Error
                ? error
            ...
            Logger.error(
                LogModule.RAG_INJECT,
                `工作流执行异常: ${workflow.name}`,
                {
                    jobId: context.id,
                    // P0 Fix: 优先记录正在执行的 step，避免误报上一个成功 step
                    step: context.metadata.currentStep || currentStepName ||
                        context.metadata.stepsExecuted.at(-1),

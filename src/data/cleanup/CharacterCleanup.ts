...
export class CharacterDeleteService {
    ...
    private static async onCharacterDeleted(
        ...
        const characterName = characterData?.name || characterData?.avatar ||
            characterData?.ch_name || characterData?.data?.name;
        ...
        if (settings.deleteIndexedDB) {
            try {
                ...
                if (matchedChatIds.length > 0) {
                    Logger.info(
                        LogModule.DATA_CLEANUP,
                        `发现 ${matchedChatIds.length} 个关联聊天数据`,
                        matchedChatIds,
                    );
                }
            } catch (error) {
        ...
        if (
            ...
            (booksToDelete.length > 0 || matchedChatIds.length > 0)
        ) {
            const confirmHtml = `
                ...
                matchedChatIds.length > 0
                    ? `
                        ...
                        <ul style="max-height: 80px; overflow-y: auto; background: var(--black50a); padding: 5px; border-radius: 4px; list-style: none; margin: 5px 0;">
                            ${
                        matchedChatIds.slice(0, 5).map((id) =>
                            `<li style="padding: 2px 0;">• ${id}</li>`
                        ).join("")
                    }
                            ${
                        matchedChatIds.length > 5
                            ? `<li style="opacity: 0.5;">... 以及其他 ${
                                matchedChatIds.length - 5
                            } 个聊天</li>`
                            : ""
                    }
     ...
     * 查找 Engram 世界书
     */
    private static async findEngramWorldbooks(
     ...
     * 执行世界书删除 (内部逻辑提取)
     */
    private static async executeDeleteWorldbooks(
        characterName: string,
     ...
     * 从 chatId 提取角色名
     */
    private static extractCharacterNameFromChatId(
        chatId: string,
    ): string | null {
            ...
            const confirmHtml = `
                <div style="font-size: 0.9em;">
                    ...
                    <ul style="max-height: 100px; overflow-y: auto; background: var(--black50a); padding: 5px; border-radius: 4px; list-style: none; margin: 10px 0;">
                        ${
                booksToDelete.map((name) =>
                    `<li style="padding: 2px 0;">• ${name}</li>`
                ).join("")
            }
                    </ul>
                    <p>是否一并删除？</p>

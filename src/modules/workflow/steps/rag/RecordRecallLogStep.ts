...
export class RecordRecallLogStep implements IStep {
    ...
    async execute(context: JobContext): Promise<void> {
        ...
        const unifiedQueries = context.input?.unifiedQueries as
            | string[]
            | undefined;
        ...
        try {
            ...
            const worldInfoText = await WorldbookScannerService
        ...
        RecallLogService.log({
            ...
            stats: {
                latencyMs: (context.data.vectorRetrieveTime || 0) +
                    (context.data.keywordRetrieveTime || 0) +
                    (context.data.rerankTime || 0),
                rerankCount: candidates.length,
                topKCount: candidates.length,
                totalCandidates: context.data.originalCandidateCount ||
                    candidates.length,
            },
        });

        const totalTime = (context.data.vectorRetrieveTime || 0) +
            (context.data.keywordRetrieveTime || 0) +
            (context.data.rerankTime || 0);

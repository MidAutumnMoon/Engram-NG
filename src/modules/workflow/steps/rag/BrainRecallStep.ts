...
export class BrainRecallStep implements IStep {
    ...
    async execute(context: JobContext): Promise<void> {
        ...
        const brainConfig: BrainRecallConfig = config.brainRecall ||
            DEFAULT_BRAIN_RECALL_CONFIG;
        const keywordEntityIds: { id: string; score: number }[] =
        ...
        if (candidates.length === 0 && keywordEntityIds.length === 0) {
            Logger.info(
                ...
                "没有检索到任何事件或实体，跳过 BrainRecallStep",
            );
            return;
        }
        ...
        if (brainConfig.enabled) {
            ...
            const mappedCandidates: RecallCandidate[] = candidates.map((c) => {
                ...
                if (rerankScore === undefined && config.useRerank) {
                    const baseScore = typeof c.embeddingScore === "number"
                        ? c.embeddingScore
                        : 0;
                ...
                return {
                    ...
                    id: c.id,
                    label: c.node?.structured_kv?.event ||
                        (c.summary ? c.summary.slice(0, 10) : "event"),
            ...
            context.data.candidates = brainResults
                ...
                .map((slot) => {
                    ...
                    const originalScore = original.hybridScore ??
                        (original.embeddingScore || 0);
            ...
            const recalledEntities = keywordEntityIds
                ...
                .map((ke) => {
                    const entity = entityMap.get(ke.id)!;
                    const brainSlot = brainResults.find((s) =>
                        s.id === entity.id
                    );
                    const floor = 0.35;
                    ...
                    return {
                        ...entity,
                        _recallWeight: brainSlot
                            ? brainSlot.finalScore
                            : clamped,
                    };
                });

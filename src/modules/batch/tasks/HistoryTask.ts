...
export class HistoryTask implements IBatchTaskHandler {
    ...
    async estimate(): Promise<BatchTask[]> {
        const targetTypes = new Set(
            this.types || ["summary", "entity", "trim", "embed"],
        );
        ...
        const currentFloor = state.current_floor ||
            MacroService.getCurrentMessageCount() || 0;
        ...
        const trimTasksCount = (targetTypes.has("trim") && trimConfig.enabled)
            ? Math.floor(summaryTasksCount / trimThreshold)
            : 0;
        // 粗略估算 embed 次数
        const embedTasksCount = targetTypes.has("embed")
            ? (summaryTasksCount || 1) * 2
            : 0;
    ...
    async *execute(
        ...
        for (let taskIndex = 0; taskIndex < tasks.length; taskIndex++) {
            ...
            try {
                ...
                Logger.error(LogModule.BATCH, `任务 ${task.type} 执行失败`, {
                    error: error.message,
                });
    ...
    private async *executeSummary(
        ...
        const summaryInterval = summarizerService.getConfig().floorInterval ||
            10;
        ...
        while (processedFloors < totalToProcess) {
            ...
            updateProgress(
                ...
                Math.min(
                    task.progress.total,
                    Math.ceil(processedFloors / summaryInterval),
                ),
            );
        }
    }
    ...
    private async *executeEntity(
        ...
        while (processedFloors < task.floorRange.end - task.floorRange.start) {
            ...
            const currentSliceEnd = Math.min(
                task.floorRange.end,
                currentSliceStart + entityInterval - 1,
            );
            ...
            try {
                ...
                const res = await entityBuilder.extractByRange([
                    currentSliceStart,
                    currentSliceEnd,
                ], true);
                if (res && !res.success) {
            ...
            updateProgress(
                ...
                Math.min(
                    task.progress.total,
                    Math.ceil(processedFloors / entityInterval),
                ),
            );
        }
    }
        ...
        for (let i = 0; i < task.progress.total; i++) {
            ...
            try {
                ...
                if (res === null) {
                    Logger.warn(
                        LogModule.BATCH,
                        "Trim skipped or returned null",
                    );
                }
            } catch (error: any) {
                Logger.error(LogModule.BATCH, "Trim failed", {
                    error: error.message,
                });
            }
            yield; // 释放控制权
    ...
    private async *executeEmbed(
        ...
        if (checkStopSignal()) return;
        const res = await embeddingService.embedUnprocessedEvents(
            (current, total) => {
                ...
                updateProgress(
                    ...
                    Math.min(
                        task.progress.total,
                        Math.ceil((current / total) * task.progress.total),
                    ),
                );
            },
        );

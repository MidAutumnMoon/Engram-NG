...
class BatchProcessor {
    ...
    async analyzeHistory(
        ...
        try {
            ...
            // 返回兼容旧 API 的格式给 UI
            return {
                archiveTasks: previewTasks.find((t) =>
                    t.type === "archive"
                )?.progress.total || 0,
                embedTasks: previewTasks.find((t) =>
                    t.type === "embed"
                )?.progress.total || 0,
                endFloor: task.endFloor,
                entityTasks: previewTasks.find((t) =>
                    t.type === "entity"
                )?.progress.total || 0,
                startFloor: task.startFloor,
                summaryTasks: previewTasks.find((t) =>
                    t.type === "summary"
                )?.progress.total || 0,
                trimTasks: previewTasks.find((t) =>
                    t.type === "trim"
                )?.progress.total || 0,
            };
        } catch (error: any) {
    ...
    async startHistory(
        ...
        if (finalState.tasks.some((t) => t.status === "error")) {
            notificationService.error(
                "批量处理结束，部分任务失败，请查看控制台",
                "Engram Batch",
            );
        } else if (
            !finalState.isPaused && !finalState.isRunning &&

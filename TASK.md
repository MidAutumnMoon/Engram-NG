...
5. **Error isolation (caught in post-T6 review).** The old two-`createRoot` design isolated PanelRoot crashes: a render error in a view unmounted only the panel root; `ReviewContainer` and `QuickPanel` (in the other root) kept working. After merging to one root, an unhandled render error in `PanelRoot` would tear down the entire `EngramRoot` tree, taking `ReviewContainer`'s `EventBus` subscription and `QuickPanel` with it. Fixed by adding `src/ui/root/PanelErrorBoundary.tsx` (class component — only class components can be error boundaries) wrapping the lazy panel in `EngramRoot`. The boundary renders `null` on error (panel appears closed), resets when `panelOpen` flips back to false, and logs to `console.error`. This restores the old isolation: a view crash leaves the rest of the Engram UI alive.

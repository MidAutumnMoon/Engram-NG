...
} from "@/modules/rag/retrieval/BrainRecallCache";
...
describe("BrainRecallCache", () => {
    ...
    describe("基础行为", () => {
        ...
        it("结果应按 finalScore 降序排列", () => {
            ...
            for (let i = 1; i < result.length; i++) {
                ...
                expect(result[i - 1].finalScore).toBeGreaterThanOrEqual(
                    result[i].finalScore - 0.3,
                );
            }
        });
    ...
    describe("强化 (Reinforce)", () => {
        it("重复召回应增强 rerankStrength", () => {
            ...
            const strengthAfter =
                snapshotAfter.find((s) => s.id === "evt_001")!.rerankStrength;
        ...
        it("embeddingStrength 应取 max(旧值, 新值)", () => {
            ...
            const slot = cache.getShortTermSnapshot().find((s) =>
                s.id === "evt_001"
            )!;
    ...
    describe("衰减 (Decay)", () => {
        it("未被召回的记忆应衰减", () => {
            ...
            const before = cache.getShortTermSnapshot().find((s) =>
                s.id === "evt_001"
            )!;
            ...
            const after = cache.getShortTermSnapshot().find((s) =>
                s.id === "evt_001"
            )!;
    ...
    describe("Decay Bomb (上下文切换)", () => {
        it("全新候选（零重叠）应触发 Decay Bomb", () => {
            ...
            const oldScore1 =
                beforeBomb.find((s) => s.id === "evt_old_1")!.rerankStrength;
            const oldScore2 =
                beforeBomb.find((s) => s.id === "evt_old_2")!.rerankStrength;
        ...
        it("重叠度高时不应触发 Decay Bomb", () => {
            ...
            const scoreAfter =
                cache.getShortTermSnapshot().find((s) => s.id === "evt_001")!
                    .finalScore;

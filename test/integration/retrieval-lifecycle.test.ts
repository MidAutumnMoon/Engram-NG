...
describe("Retrieval Lifecycle Integration", () => {
    ...
    it("should retrieve events via vector search and apply brain recall decay", async () => {
        ...
        // 3. 执行第一次检索
        console.error(
            `DEBUG: it[retrieve events] starting with retriever_ID=[${
                (retriever as any).instanceId
            }] embeddingService_ID=[${(embeddingService as any).instanceId}]`,
        );
        console.error("DEBUG: it[retrieve events] calling retriever.search...");
        const result1 = await retriever.search("我在找关于跑步的事情");
        console.error("DEBUG: it[retrieve events] search finished", {
            count: result1.nodes.length,
        });
